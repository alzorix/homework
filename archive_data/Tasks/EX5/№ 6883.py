''' 	(№ 6883) (К. Багдасарян) Алгоритм получает на вход натуральное число N > 4 и строит по нему новое число R следующим образом:
...
for N in range( 5,999):
    fiveN = num(N)
    if N %5 ==0:
        fiveN = fiveN + fiveN[len(str(N))-2:]
    else:
        fiveN = fiveN + num((N%5)*7)
